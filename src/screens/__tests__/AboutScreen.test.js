...
jest.mock('../../data/data', () => ({
  interests: ['gardening', 'cross-country running'],
  technologies: ['advanced neural networks', 'integration layer development']
}))
...
describe('<AboutScreen />', () => {
  ...
  it('should render the AboutScreen with a list of technologies', () => {
    expect(component.find('.technology').first().dive().text()).toEqual(
      'advanced neural networks'
    )
    expect(component.find('.technology').at(1).dive().text()).toEqual(
      'integration layer development'
    )
  })

  it('should render the AboutScreen with a list of interests', () => {
    expect(component.find('.interest').first().dive().text()).toEqual(
      'gardening'
    )
    expect(component.find('.interest').at(1).dive().text()).toEqual(
      'cross-country running'
    )
  })

  it('should render the AboutScreen with a link to my website', () => {
    expect(component.find('.website').dive().text()).toEqual(
      ' For More Information, Visit My '
    )
    expect(component.find('ForwardRef').children().dive().text()).toEqual(
      'Website'
    )
  })
})
